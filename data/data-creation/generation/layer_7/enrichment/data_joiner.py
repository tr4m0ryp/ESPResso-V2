"""Data joiner for Layer 7: joins Layer 5 validated records
...
This module is a direct copy of the Layer 6 data_joiner logic,
adapted for Layer 7 water footprint calculation.
...
_LAYER4_COLUMNS = [
    'preprocessing_path_id', 'transport_legs', 'total_distance_km',
]

# Expected minimum row count after join
...
def _verify_files_exist(
    layer5_path: str, layer4_path: str
) -> None:
...
def _load_layer5(layer5_path: str) -> pd.DataFrame:
    ...
    Returns:
        DataFrame with all original columns plus pp_id.
    """
    logger.info("Loading Layer 5 dataset from %s", layer5_path)
    df = pd.read_csv(layer5_path)
    logger.info(
        "Loaded %d Layer 5 records (%d columns)",
...
def _load_layer4(layer4_path: str) -> pd.DataFrame:
    ...
    Returns:
        ...
        total_distance_km columns, deduplicated.
    """
    logger.info(
        "Loading Layer 4 transport_legs from %s", layer4_path
    )
    ...
    if 'preprocessing_path_id' not in df.columns:
        raise ValueError(
            f"Layer 4 parquet missing 'preprocessing_path_id'. "
            f"Available columns: {list(df.columns)}"
        )
...
def _join(
    ...
    Args:
        layer5_df: Layer 5 DataFrame with pp_id column.
        layer4_df: Layer 4 DataFrame with preprocessing_path_id.

    Returns:
        Merged DataFrame with helper columns dropped.
    """
    ...
    cols_to_drop = [
        c for c in ('pp_id', 'preprocessing_path_id')
...
def _check_unmatched(
    ...
    """Log warnings for unmatched Layer 5 records.

    Args:
        layer5_df: Original Layer 5 DataFrame (with pp_id).
    ...
    if unmatched_count > 0:
        logger.warning(
            "%d Layer 5 records unmatched (%d/%d matched)",
            unmatched_count, matched_count, original_count
        )
    else:
        logger.info(
            "All %d Layer 5 records matched Layer 4",
            matched_count
        )


def _validate_row_count(merged: pd.DataFrame) -> None:
    """Raise ValueError if row count is below expected minimum.
    ...
    if row_count < _EXPECTED_MIN_ROWS:
        raise ValueError(
            f"Join produced only {row_count} rows, expected "
            f">= {_EXPECTED_MIN_ROWS}. Check Layer 4 contains "
            f"matching preprocessing_path_id values."
        )
    logger.info(
        "Row count validation passed: %d rows (>= %d minimum)",
