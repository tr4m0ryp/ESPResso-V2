 ...
 * combinations from CSV datasets, category string conversion,
...
/** @brief CSV column indices for processing_steps_overview.csv. */
#define STEP_COL_NAME 0
#define STEP_COL_PROCESS_ID 1
#define STEP_COL_CATEGORY 2
#define STEP_COL_EMISSION_FACTOR 3
#define STEP_COL_APPLICABLE_MATERIALS 4
#define STEP_COL_DESCRIPTION 5

/** @brief CSV column indices for material_processing_emissions.csv. */
...
#define COMBO_COL_EMISSION_FACTOR 8
                              ...
                              const char *filepath)
{
    ...
    while ((result = csv_parser_read_row(&parser, &row)) == 1) {
        ...
        if (csv_parser_get_field_double(
                &row, STEP_COL_EMISSION_FACTOR,
                &step->emission_factor_kg_CO2e_per_kg)
                != 0) {
            step->emission_factor_kg_CO2e_per_kg = 0.0;
        }
    ...
    MaterialProcessDatabase *db, const char *filepath)
{
    ...
    while ((result = csv_parser_read_row(&parser, &row)) == 1) {
        ...
        if (csv_parser_get_field_double(
                &row, COMBO_COL_EMISSION_FACTOR,
                &combo->emission_factor_kg_CO2e_per_kg)
                != 0) {
            combo->emission_factor_kg_CO2e_per_kg = 0.0;
        }
...
void processing_free_step_database(
    ProcessingStepDatabase *db)
{
    if (db != NULL) {
        db->count = 0;
    }
...
void processing_free_combo_database(
    MaterialProcessDatabase *db)
{
    if (db != NULL) {
        db->count = 0;
    }
